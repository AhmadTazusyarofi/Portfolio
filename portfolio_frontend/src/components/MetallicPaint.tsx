  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = function () {
      ...
      if (file.type === "image/svg+xml") {
        // SVG kadang nggak punya natural size → paksa square
        width = 512;
        height = 512;
      }
      ...
      const C = 0.01;
      const ITERATIONS = 100; // ⚡ dikurangi dari 300 → 100
...
// === fragment shader (punyamu, tidak diubah) ===
...
export default function MetallicPaint({
  ...
  function updateUniforms(
    ...
    glCtx.uniform1f(u.u_edge, params.edge);
    glCtx.uniform1f(u.u_patternBlur, params.patternBlur);
    glCtx.uniform1f(u.u_time, 0);
    glCtx.uniform1f(u.u_patternScale, params.patternScale);
    glCtx.uniform1f(u.u_refraction, params.refraction);
    glCtx.uniform1f(u.u_liquid, params.liquid);
  }
  ...
  useEffect(() => {
    function initShader() {
      ...
      if (!glCtx.getProgramParameter(program, glCtx.LINK_STATUS)) {
        console.error(
          "Unable to initialize the shader program: " +
            glCtx.getProgramInfoLog(program)
        );
        return;
      }
      ...
      function getUniforms(
        ...
        const uniformCount = glCtx.getProgramParameter(
          program,
        ...
        for (let i = 0; i < uniformCount; i++) {
          ...
          const location = glCtx.getUniformLocation(program, uniformName);
          if (location) {
      ...
      const uniformsObj = getUniforms(program, glCtx);
      setUniforms(uniformsObj);
  ...
  // Update uniforms ketika gl / uniforms siap
  useEffect(() => {
    if (!gl) return;
  ...
  // Resize canvas + set ratio
  useEffect(() => {
    if (!gl) return;
    if (!uniforms || !uniforms.u_ratio || !uniforms.u_img_ratio) return;
    if (!imageData) return;
  ...
  // Upload texture (bagian yang error tadi)
  useEffect(() => {
    if (!gl) return;
    if (!uniforms || !uniforms.u_image_texture) return;
    if (!imageData) return;
    ...
    return () => {
      // 👇 di sini tadi TS protes → sekarang glCtx sudah dipaksa non-null
      glCtx.deleteTexture(imageTexture);
    };
  }, [gl, uniforms, imageData]);
