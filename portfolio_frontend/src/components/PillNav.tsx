...
export interface PillNavProps {
  ...
  logoAlt?: string;
  /** Tujuan klik logo. Default: href item pertama. */
  logoHref?: string;
  items: PillNavItem[];
  activeHref?: string;
  className?: string;
  ...
  /** Dipanggil sebelum navigasi default. Panggil preventDefault() di dalamnya
   *  untuk mengambil alih navigasi (mis. lewat router) tanpa memuat ulang. */
  onItemClick?: (
    href: string,
    ...
    <div className="fixed top-[1em] z-1000 w-full left-0 md:left-1/2 md:w-max md:-translate-x-1/2">
      ...
      >
        <a
          ...
          onClick={(e) =>
            onItemClick?.(logoHref ?? items?.[0]?.href ?? "#", e)
          }
          ref={logoRef}
